...
$d_nilai_grid = NULL; // Initialize page object first

class cd_nilai_grid extends cd_nilai {
	...
	// Page object name
	var $PageObjName = 'd_nilai_grid';

	// Grid form hidden field names
	var $FormName = 'fd_nilaigrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["d_nilai"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->detail_nilai_id->CurrentValue;
	...
	function EmptyRow() {
		...
		if ($objForm->HasValue("x_nilai_psikomotorik") && $objForm->HasValue("o_nilai_psikomotorik") && $this->nilai_psikomotorik->CurrentValue <> $this->nilai_psikomotorik->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilai_rata2Drata") && $objForm->HasValue("o_nilai_rata2Drata") && $this->nilai_rata2Drata->CurrentValue <> $this->nilai_rata2Drata->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->detail_nilai_id->CurrentValue . "\">";
		}
	...
	function LoadFormValues() {
		...
		$this->nilai_rata2Drata->setOldValue($objForm->GetValue("o_nilai_rata2Drata"));
		if (!$this->detail_nilai_id->FldIsDetailKey && $this->CurrentAction <> "gridadd" && $this->CurrentAction <> "add")
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->detail_nilai_id->CurrentValue = strval($arKeys[0]); // detail_nilai_id
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "nilai") {
				$this->nilai_id->CurrentValue = $this->nilai_id->getSessionValue();
			}
