...
$d_nilai_list = NULL; // Initialize page object first

class cd_nilai_list extends cd_nilai {
	...
	// Page object name
	var $PageObjName = 'd_nilai_list';

	// Grid form hidden field names
	var $FormName = 'fd_nilailist';
	...
	function __construct() {
		...
		$this->AddUrl = "d_nilaiadd.php";
		$this->InlineAddUrl = $this->PageUrl() . "a=add";
		$this->GridAddUrl = $this->PageUrl() . "a=gridadd";
		$this->GridEditUrl = $this->PageUrl() . "a=gridedit";
		$this->MultiDeleteUrl = "d_nilaidelete.php";
		$this->MultiUpdateUrl = "d_nilaiupdate.php";
	...
	function SetupKeyValues($key) {
		...
		if (count($arrKeyFlds) >= 1) {
			$this->detail_nilai_id->setFormValue($arrKeyFlds[0]);
			if (!is_numeric($this->detail_nilai_id->FormValue))
				return FALSE;
		}
		return TRUE;
	}
	...
	function SetUpSortOrder() {
		...
		if (@$_GET["order"] <> "") {
			...
			$this->UpdateSort($this->nilai_rata2Drata); // nilai_rata-rata
	...
	function RenderListOptions() {
		...
		$oListOpt->Body = "<label class=\"checkbox\"><input type=\"checkbox\" name=\"key_m[]\" value=\"" . ew_HtmlEncode($this->detail_nilai_id->CurrentValue) . "\" onclick='ew_ClickMultiCheckbox(event, this);'></label>";
	...
	function RenderOtherOptions() {
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->CustomActions as $action => $name) {
				...
				$item->Body = "<a class=\"ewAction ewCustomAction\" href=\"\" onclick=\"ew_SubmitSelected(document.fd_nilailist, '" . ew_CurrentUrl() . "', null, '" . $action . "');return false;\">" . $name . "</a>";
			}
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_d_nilai\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_d_nilai',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fd_nilailist,sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($d_nilai_list)) $d_nilai_list = new cd_nilai_list();

// Page init
$d_nilai_list->Page_Init();

// Page main
$d_nilai_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$d_nilai_list->Page_Render();
?>
...
// Page object
var d_nilai_list = new ew_Page("d_nilai_list");
d_nilai_list.PageID = "list"; // Page ID
var EW_PAGE_ID = d_nilai_list.PageID; // For backward compatibility

// Form object
var fd_nilailist = new ew_Form("fd_nilailist");
fd_nilailist.FormKeyCountName = '<?php echo $d_nilai_list->FormKeyCountName ?>';

// Form_CustomValidate event
fd_nilailist.Form_CustomValidate = 
...
fd_nilailist.ValidateRequired = true;
<?php } else { ?>
fd_nilailist.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fd_nilailist.Lists["x_id_mapel"] = {"LinkField":"x_id_mapel","Ajax":null,"AutoFill":false,"DisplayFields":["x_namaMapel","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<?php if ($d_nilai->getCurrentMasterTable() == "" && $d_nilai_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $d_nilai_list->ExportOptions->Render("body") ?></div>
<?php } ?>
<?php if (($d_nilai->Export == "") || (EW_EXPORT_MASTER_RECORD && $d_nilai->Export == "print")) { ?>
<?php
$gsMasterReturnUrl = "nilailist.php";
if ($d_nilai_list->DbMasterFilter <> "" && $d_nilai->getCurrentMasterTable() == "nilai") {
	if ($d_nilai_list->MasterRecordExists) {
		if ($d_nilai->getCurrentMasterTable() == $d_nilai->TableVar) $gsMasterReturnUrl .= "?" . EW_TABLE_SHOW_MASTER . "=";
?>
<?php if ($d_nilai_list->ExportOptions->Visible()) { ?>
<div class="ewListExportOptions"><?php $d_nilai_list->ExportOptions->Render("body") ?></div>
<?php } ?>
...
<?php
	...
	if ($bSelectLimit) {
		$d_nilai_list->TotalRecs = $d_nilai->SelectRecordCount();
	} else {
		if ($d_nilai_list->Recordset = $d_nilai_list->LoadRecordset())
			$d_nilai_list->TotalRecs = $d_nilai_list->Recordset->RecordCount();
	}
	$d_nilai_list->StartRec = 1;
	if ($d_nilai_list->DisplayRecs <= 0 || ($d_nilai->Export <> "" && $d_nilai->ExportAll)) // Display all records
		$d_nilai_list->DisplayRecs = $d_nilai_list->TotalRecs;
	if (!($d_nilai->Export <> "" && $d_nilai->ExportAll))
		$d_nilai_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$d_nilai_list->Recordset = $d_nilai_list->LoadRecordset($d_nilai_list->StartRec-1, $d_nilai_list->DisplayRecs);
$d_nilai_list->RenderOtherOptions();
?>
<?php $d_nilai_list->ShowPageHeader(); ?>
<?php
$d_nilai_list->ShowMessage();
?>
<table class="ewGrid"><tr><td class="ewGridContent">
<?php if ($d_nilai->Export == "") { ?>
<div class="ewGridUpperPanel">
<?php if ($d_nilai->CurrentAction <> "gridadd" && $d_nilai->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($d_nilai_list->Pager)) $d_nilai_list->Pager = new cPrevNextPager($d_nilai_list->StartRec, $d_nilai_list->DisplayRecs, $d_nilai_list->TotalRecs) ?>
<?php if ($d_nilai_list->Pager->RecordCount > 0) { ?>
<table class="ewStdTable"><tbody><tr><td>
	<?php echo $Language->Phrase("Page") ?>&nbsp;
<div class="input-prepend input-append">
<!--first page button-->
	<?php if ($d_nilai_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $d_nilai_list->PageUrl() ?>start=<?php echo $d_nilai_list->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-backward"></i></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($d_nilai_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $d_nilai_list->PageUrl() ?>start=<?php echo $d_nilai_list->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-prev"></i></a>
	<?php } ?>
<!--current page number-->
	<input class="input-mini" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $d_nilai_list->Pager->CurrentPage ?>">
<!--next page button-->
	<?php if ($d_nilai_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $d_nilai_list->PageUrl() ?>start=<?php echo $d_nilai_list->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($d_nilai_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $d_nilai_list->PageUrl() ?>start=<?php echo $d_nilai_list->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-forward"></i></a>
	<?php } ?>
</div>
	&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $d_nilai_list->Pager->PageCount ?>
</td>
<td>
	&nbsp;&nbsp;&nbsp;&nbsp;
	<?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $d_nilai_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $d_nilai_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $d_nilai_list->Pager->RecordCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
	<?php if ($d_nilai_list->SearchWhere == "0=101") { ?>
...
<?php if ($d_nilai_list->TotalRecs > 0) { ?>
<td>
	&nbsp;&nbsp;&nbsp;&nbsp;
...
<option value="10"<?php if ($d_nilai_list->DisplayRecs == 10) { ?> selected="selected"<?php } ?>>10</option>
<option value="20"<?php if ($d_nilai_list->DisplayRecs == 20) { ?> selected="selected"<?php } ?>>20</option>
<option value="50"<?php if ($d_nilai_list->DisplayRecs == 50) { ?> selected="selected"<?php } ?>>50</option>
<option value="ALL"<?php if ($d_nilai->getRecordsPerPage() == -1) { ?> selected="selected"<?php } ?>><?php echo $Language->Phrase("AllRecords") ?></option>
</select>
</td>
<?php } ?>
</tr></table>
...
<?php
	foreach ($d_nilai_list->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
</div>
<?php } ?>
<form name="fd_nilailist" id="fd_nilailist" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="d_nilai">
<div id="gmp_d_nilai" class="ewGridMiddlePanel">
<?php if ($d_nilai_list->TotalRecs > 0) { ?>
<table id="tbl_d_nilailist" class="ewTable ewTableSeparate">
...
// Render list options
$d_nilai_list->RenderListOptions();

// Render list options (header, left)
$d_nilai_list->ListOptions->Render("header", "left");
?>
<?php if ($d_nilai->id_mapel->Visible) { // id_mapel ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $d_nilai->SortUrl($d_nilai->id_mapel) ?>',1);"><div id="elh_d_nilai_id_mapel" class="d_nilai_id_mapel">
...
<?php if ($d_nilai->nilai_afektif->Visible) { // nilai_afektif ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $d_nilai->SortUrl($d_nilai->nilai_afektif) ?>',1);"><div id="elh_d_nilai_nilai_afektif" class="d_nilai_nilai_afektif">
...
<?php if ($d_nilai->nilai_komulatif->Visible) { // nilai_komulatif ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $d_nilai->SortUrl($d_nilai->nilai_komulatif) ?>',1);"><div id="elh_d_nilai_nilai_komulatif" class="d_nilai_nilai_komulatif">
...
<?php if ($d_nilai->nilai_psikomotorik->Visible) { // nilai_psikomotorik ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $d_nilai->SortUrl($d_nilai->nilai_psikomotorik) ?>',1);"><div id="elh_d_nilai_nilai_psikomotorik" class="d_nilai_nilai_psikomotorik">
...
<?php if ($d_nilai->nilai_rata2Drata->Visible) { // nilai_rata-rata ?>
	...
	<?php } else { ?>
		<td><div class="ewPointer" onclick="ew_Sort(event,'<?php echo $d_nilai->SortUrl($d_nilai->nilai_rata2Drata) ?>',1);"><div id="elh_d_nilai_nilai_rata2Drata" class="d_nilai_nilai_rata2Drata">
...
$d_nilai_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($d_nilai->ExportAll && $d_nilai->Export <> "") {
	$d_nilai_list->StopRec = $d_nilai_list->TotalRecs;
} else {

	// Set the last record to display
	if ($d_nilai_list->TotalRecs > $d_nilai_list->StartRec + $d_nilai_list->DisplayRecs - 1)
		$d_nilai_list->StopRec = $d_nilai_list->StartRec + $d_nilai_list->DisplayRecs - 1;
	else
		$d_nilai_list->StopRec = $d_nilai_list->TotalRecs;
}
$d_nilai_list->RecCnt = $d_nilai_list->StartRec - 1;
if ($d_nilai_list->Recordset && !$d_nilai_list->Recordset->EOF) {
	$d_nilai_list->Recordset->MoveFirst();
	if (!$bSelectLimit && $d_nilai_list->StartRec > 1)
		$d_nilai_list->Recordset->Move($d_nilai_list->StartRec - 1);
} elseif (!$d_nilai->AllowAddDeleteRow && $d_nilai_list->StopRec == 0) {
	$d_nilai_list->StopRec = $d_nilai->GridAddRowCount;
}

// Initialize aggregate
$d_nilai->RowType = EW_ROWTYPE_AGGREGATEINIT;
$d_nilai->ResetAttrs();
$d_nilai_list->RenderRow();
while ($d_nilai_list->RecCnt < $d_nilai_list->StopRec) {
	$d_nilai_list->RecCnt++;
	if (intval($d_nilai_list->RecCnt) >= intval($d_nilai_list->StartRec)) {
		$d_nilai_list->RowCnt++;

		// Set up key count
		$d_nilai_list->KeyCount = $d_nilai_list->RowIndex;
		...
		if ($d_nilai->CurrentAction == "gridadd") {
			...
			$d_nilai_list->LoadRowValues($d_nilai_list->Recordset); // Load row values
		}
		$d_nilai->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$d_nilai->RowAttrs = array_merge($d_nilai->RowAttrs, array('data-rowindex'=>$d_nilai_list->RowCnt, 'id'=>'r' . $d_nilai_list->RowCnt . '_d_nilai', 'data-rowtype'=>$d_nilai->RowType));
...
$d_nilai_list->ListOptions->Render("body", "left", $d_nilai_list->RowCnt);
?>
	<?php if ($d_nilai->id_mapel->Visible) { // id_mapel ?>
...
<a id="<?php echo $d_nilai_list->PageObjName . "_row_" . $d_nilai_list->RowCnt ?>"></a></td>
	<?php } ?>
	<?php if ($d_nilai->nilai_afektif->Visible) { // nilai_afektif ?>
...
$d_nilai_list->ListOptions->Render("body", "right", $d_nilai_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($d_nilai->CurrentAction <> "gridadd")
		$d_nilai_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($d_nilai->CurrentAction == "") { ?>
...
// Close recordset
if ($d_nilai_list->Recordset)
	$d_nilai_list->Recordset->Close();
?>
<?php if ($d_nilai_list->TotalRecs > 0) { ?>
<?php if ($d_nilai->Export == "") { ?>
