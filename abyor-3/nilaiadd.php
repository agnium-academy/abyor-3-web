...
class cnilai_add extends cnilai {
	...
	function ValidateForm() {
		...
		if (in_array("d_nilai", $DetailTblVar) && $GLOBALS["d_nilai"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("d_nilai", $DetailTblVar) && $GLOBALS["d_nilai"]->DetailAdd) {
				$GLOBALS["d_nilai"]->nilai_id->setSessionValue($this->nilai_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["d_nilai_grid"])) $GLOBALS["d_nilai_grid"] = new cd_nilai_grid(); // Get detail page object
				$AddRow = $GLOBALS["d_nilai_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["d_nilai"]->nilai_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("d_nilai", $DetailTblVar)) {
				...
				if ($GLOBALS["d_nilai_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["d_nilai_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["d_nilai_grid"]->CurrentMode = "add";
					$GLOBALS["d_nilai_grid"]->CurrentAction = "gridadd";
...
fnilaiadd.Lists["x_nis"] = {"LinkField":"x_nis","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fnilaiadd.Lists["x_nip"] = {"LinkField":"x_nip","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php
	if (in_array("d_nilai", explode(",", $nilai->getCurrentDetailTable())) && $d_nilai->DetailAdd) {
?>
