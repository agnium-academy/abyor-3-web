...
$detail_nilai_grid = NULL; // Initialize page object first

class cdetail_nilai_grid extends cdetail_nilai {
	...
	// Page object name
	var $PageObjName = 'detail_nilai_grid';

	// Grid form hidden field names
	var $FormName = 'fdetail_nilaigrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detail_nilai"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
		...
		for ($rowindex = 1; $rowindex <= $rowcnt; $rowindex++) {
			...
			if (!$this->EmptyRow()) {
				...
				if ($bGridInsert) {
					...
					$sKey .= $this->id_detail_nilai->CurrentValue;
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_id_mapel") && $objForm->HasValue("o_id_mapel") && $this->id_mapel->CurrentValue <> $this->id_mapel->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilai_afektif") && $objForm->HasValue("o_nilai_afektif") && $this->nilai_afektif->CurrentValue <> $this->nilai_afektif->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilai_komulatif") && $objForm->HasValue("o_nilai_komulatif") && $this->nilai_komulatif->CurrentValue <> $this->nilai_komulatif->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilai_psikomotorik") && $objForm->HasValue("o_nilai_psikomotorik") && $this->nilai_psikomotorik->CurrentValue <> $this->nilai_psikomotorik->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_nilai_rata_rata") && $objForm->HasValue("o_nilai_rata_rata") && $this->nilai_rata_rata->CurrentValue <> $this->nilai_rata_rata->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function RenderListOptions() {
		...
		if ($this->CurrentMode == "edit" && is_numeric($this->RowIndex)) {
			$this->MultiSelectKey .= "<input type=\"hidden\" name=\"" . $KeyName . "\" id=\"" . $KeyName . "\" value=\"" . $this->id_detail_nilai->CurrentValue . "\">";
		}
	...
	function LoadDefaultValues() {
		...
		$this->nilai_rata_rata->CurrentValue = NULL;
		$this->nilai_rata_rata->OldValue = $this->nilai_rata_rata->CurrentValue;
	}
	...
	function LoadOldRecord() {
		...
		if ($cnt >= 1) {
			if (strval($arKeys[0]) <> "")
				$this->id_detail_nilai->CurrentValue = strval($arKeys[0]); // id_detail_nilai
			else
				$bValidKey = FALSE;
		} else {
			$bValidKey = FALSE;
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "nilai") {
				$this->id_nilai->CurrentValue = $this->id_nilai->getSessionValue();
			}
		...
		// nilai_rata_rata
		$this->nilai_rata_rata->SetDbValueDef($rsnew, $this->nilai_rata_rata->CurrentValue, NULL, FALSE);

		// id_nilai
		if ($this->id_nilai->getSessionValue() <> "") {
			$rsnew['id_nilai'] = $this->id_nilai->getSessionValue();
		}
		...
		if ($AddRow) {
			$this->id_detail_nilai->setDbValue($conn->Insert_ID());
			$rsnew['id_detail_nilai'] = $this->id_detail_nilai->DbValue;
		}
		if ($AddRow) {
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "nilai") {
			$this->id_nilai->Visible = FALSE;
			if ($GLOBALS["nilai"]->EventCancelled) $this->EventCancelled = TRUE;
		}
