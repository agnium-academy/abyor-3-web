...
$detail_nilai_view = NULL; // Initialize page object first

class cdetail_nilai_view extends cdetail_nilai {
	...
	// Page object name
	var $PageObjName = 'detail_nilai_view';
	...
	function __construct() {
		...
		if (@$_GET["id_detail_nilai"] <> "") {
			$this->RecKey["id_detail_nilai"] = $_GET["id_detail_nilai"];
			$KeyUrl .= "&amp;id_detail_nilai=" . urlencode($this->RecKey["id_detail_nilai"]);
		}
	...
	function Page_Init() {
		...
		if (@$_GET["id_detail_nilai"] <> "") {
			...
			$gsExportFile .= ew_StripSlashes($_GET["id_detail_nilai"]);
		}
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id_detail_nilai"] <> "") {
				...
				$this->RecKey["id_detail_nilai"] = $this->id_detail_nilai->QueryStringValue;
			} else {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if ($this->TotalRecs <= 0) { // No record found
						...
						$this->Page_Terminate("detail_nilailist.php"); // Return to list page
						...
						while (!$this->Recordset->EOF) {
							if (strval($this->id_detail_nilai->CurrentValue) == strval($this->Recordset->fields('id_detail_nilai'))) {
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "detail_nilailist.php"; // No matching record, return to list
					} else {
			...
			$sReturnUrl = "detail_nilailist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_detail_nilai\" href=\"javascript:void(0);\" class=\"ewExportLink ewEmail\" data-caption=\"" . $Language->Phrase("ExportToEmailText") . "\" onclick=\"ew_EmailDialogShow({lnk:'emf_detail_nilai',hdr:ewLanguage.Phrase('ExportToEmail'),f:document.fdetail_nilaiview,key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($detail_nilai_view)) $detail_nilai_view = new cdetail_nilai_view();

// Page init
$detail_nilai_view->Page_Init();

// Page main
$detail_nilai_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$detail_nilai_view->Page_Render();
?>
...
// Page object
var detail_nilai_view = new ew_Page("detail_nilai_view");
detail_nilai_view.PageID = "view"; // Page ID
var EW_PAGE_ID = detail_nilai_view.PageID; // For backward compatibility

// Form object
var fdetail_nilaiview = new ew_Form("fdetail_nilaiview");

// Form_CustomValidate event
fdetail_nilaiview.Form_CustomValidate = 
...
fdetail_nilaiview.ValidateRequired = true;
<?php } else { ?>
fdetail_nilaiview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fdetail_nilaiview.Lists["x_id_mapel"] = {"LinkField":"x_id_mapel","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama_mapel","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $detail_nilai_view->ExportOptions->Render("body") ?>
<?php if (!$detail_nilai_view->ExportOptions->UseDropDownButton) { ?>
</div>
<div class="ewViewOtherOptions">
<?php } ?>
<?php
	foreach ($detail_nilai_view->OtherOptions as &$option)
		$option->Render("body");
?>
</div>
<?php } ?>
<?php $detail_nilai_view->ShowPageHeader(); ?>
<?php
$detail_nilai_view->ShowMessage();
?>
<?php if ($detail_nilai->Export == "") { ?>
<form name="ewPagerForm" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>">
<table class="ewPager">
<tr><td>
<?php if (!isset($detail_nilai_view->Pager)) $detail_nilai_view->Pager = new cPrevNextPager($detail_nilai_view->StartRec, $detail_nilai_view->DisplayRecs, $detail_nilai_view->TotalRecs) ?>
<?php if ($detail_nilai_view->Pager->RecordCount > 0) { ?>
<table class="ewStdTable"><tbody><tr><td>
	<?php echo $Language->Phrase("Page") ?>&nbsp;
<div class="input-prepend input-append">
<!--first page button-->
	<?php if ($detail_nilai_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $detail_nilai_view->PageUrl() ?>start=<?php echo $detail_nilai_view->Pager->FirstButton->Start ?>"><i class="icon-step-backward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-backward"></i></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($detail_nilai_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $detail_nilai_view->PageUrl() ?>start=<?php echo $detail_nilai_view->Pager->PrevButton->Start ?>"><i class="icon-prev"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-prev"></i></a>
	<?php } ?>
<!--current page number-->
	<input class="input-mini" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $detail_nilai_view->Pager->CurrentPage ?>">
<!--next page button-->
	<?php if ($detail_nilai_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $detail_nilai_view->PageUrl() ?>start=<?php echo $detail_nilai_view->Pager->NextButton->Start ?>"><i class="icon-play"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-play"></i></a>
	<?php } ?>
<!--last page button-->
	<?php if ($detail_nilai_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-small" href="<?php echo $detail_nilai_view->PageUrl() ?>start=<?php echo $detail_nilai_view->Pager->LastButton->Start ?>"><i class="icon-step-forward"></i></a>
	<?php } else { ?>
	<a class="btn btn-small disabled"><i class="icon-step-forward"></i></a>
	<?php } ?>
</div>
	&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $detail_nilai_view->Pager->PageCount ?>
</td>
</tr></tbody></table>
<?php } else { ?>
...
<?php } ?>
<form name="fdetail_nilaiview" id="fdetail_nilaiview" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="detail_nilai">
<table class="ewGrid"><tr><td>
<table id="tbl_detail_nilaiview" class="table table-bordered table-striped">
<?php if ($detail_nilai->id_mapel->Visible) { // id_mapel ?>
	<tr id="r_id_mapel">
		<td><span id="elh_detail_nilai_id_mapel"><?php echo $detail_nilai->id_mapel->FldCaption() ?></span></td>
...
<?php if ($detail_nilai->nilai_afektif->Visible) { // nilai_afektif ?>
	<tr id="r_nilai_afektif">
		<td><span id="elh_detail_nilai_nilai_afektif"><?php echo $detail_nilai->nilai_afektif->FldCaption() ?></span></td>
...
<?php if ($detail_nilai->nilai_komulatif->Visible) { // nilai_komulatif ?>
	<tr id="r_nilai_komulatif">
		<td><span id="elh_detail_nilai_nilai_komulatif"><?php echo $detail_nilai->nilai_komulatif->FldCaption() ?></span></td>
...
<?php if ($detail_nilai->nilai_psikomotorik->Visible) { // nilai_psikomotorik ?>
	<tr id="r_nilai_psikomotorik">
		<td><span id="elh_detail_nilai_nilai_psikomotorik"><?php echo $detail_nilai->nilai_psikomotorik->FldCaption() ?></span></td>
...
<script type="text/javascript">
fdetail_nilaiview.Init();
</script>
<?php
$detail_nilai_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($detail_nilai->Export == "") { ?>
...
<?php
$detail_nilai_view->Page_Terminate();
?>
