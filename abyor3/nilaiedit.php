...
class cnilai_edit extends cnilai {
	...
	function ValidateForm() {
		...
		if (in_array("detail_nilai", $DetailTblVar) && $GLOBALS["detail_nilai"]->DetailEdit) {
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// nis
			$this->nis->SetDbValueDef($rsnew, $this->nis->CurrentValue, 0, $this->nis->ReadOnly);

			// nip
			$this->nip->SetDbValueDef($rsnew, $this->nip->CurrentValue, 0, $this->nip->ReadOnly);
			...
			if ($bUpdateRow) {
				...
				if ($EditRow) {
					...
					if (in_array("detail_nilai", $DetailTblVar) && $GLOBALS["detail_nilai"]->DetailEdit) {
						if (!isset($GLOBALS["detail_nilai_grid"])) $GLOBALS["detail_nilai_grid"] = new cdetail_nilai_grid(); // Get detail page object
						$EditRow = $GLOBALS["detail_nilai_grid"]->GridUpdate();
					}
				}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("detail_nilai", $DetailTblVar)) {
				...
				if ($GLOBALS["detail_nilai_grid"]->DetailEdit) {
					$GLOBALS["detail_nilai_grid"]->CurrentMode = "edit";
					$GLOBALS["detail_nilai_grid"]->CurrentAction = "gridedit";
...
fnilaiedit.Lists["x_nis"] = {"LinkField":"x_id_siswa","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fnilaiedit.Lists["x_nip"] = {"LinkField":"x_guru_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php
	if (in_array("detail_nilai", explode(",", $nilai->getCurrentDetailTable())) && $detail_nilai->DetailEdit) {
?>
