...
<?php include_once "detail_nilaigridcls.php" ?>
...
$nilai_add = NULL; // Initialize page object first

class cnilai_add extends cnilai {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
			...
			if (@$_GET["nilai_id"] != "") {
				...
				$this->setKey("nilai_id", $this->nilai_id->CurrentValue); // Set up key
			} else {
				$this->setKey("nilai_id", ""); // Clear key
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$sFilterWrk = "";
			$sSqlWrk = "SELECT `id_siswa`, `nama` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `siswa`";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
				ew_AddFilter($sWhereWrk, $sFilterWrk);
			}
			...
			$sFilterWrk = "";
			$sSqlWrk = "SELECT `guru_id`, `nama` AS `DispFld`, '' AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld`, '' AS `SelectFilterFld`, '' AS `SelectFilterFld2`, '' AS `SelectFilterFld3`, '' AS `SelectFilterFld4` FROM `guru`";
			$sWhereWrk = "";
			if ($sFilterWrk <> "") {
				ew_AddFilter($sWhereWrk, $sFilterWrk);
			}
	...
	function ValidateForm() {
		...
		if (in_array("detail_nilai", $DetailTblVar) && $GLOBALS["detail_nilai"]->DetailAdd) {
			if (!isset($GLOBALS["detail_nilai_grid"])) $GLOBALS["detail_nilai_grid"] = new cdetail_nilai_grid(); // get detail page object
			$GLOBALS["detail_nilai_grid"]->ValidateGridForm();
		}
	...
	function AddRow($rsold = NULL) {
		...
		// nis
		$this->nis->SetDbValueDef($rsnew, $this->nis->CurrentValue, 0, FALSE);

		// nip
		$this->nip->SetDbValueDef($rsnew, $this->nip->CurrentValue, 0, FALSE);
		...
		if ($AddRow) {
			$this->nilai_id->setDbValue($conn->Insert_ID());
			$rsnew['nilai_id'] = $this->nilai_id->DbValue;
		}
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("detail_nilai", $DetailTblVar) && $GLOBALS["detail_nilai"]->DetailAdd) {
				$GLOBALS["detail_nilai"]->id_nilai->setSessionValue($this->nilai_id->CurrentValue); // Set master key
				if (!isset($GLOBALS["detail_nilai_grid"])) $GLOBALS["detail_nilai_grid"] = new cdetail_nilai_grid(); // Get detail page object
				$AddRow = $GLOBALS["detail_nilai_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["detail_nilai"]->id_nilai->setSessionValue(""); // Clear master key if insert failed
			}
		}
	...
	function SetUpDetailParms() {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("detail_nilai", $DetailTblVar)) {
				if (!isset($GLOBALS["detail_nilai_grid"]))
					$GLOBALS["detail_nilai_grid"] = new cdetail_nilai_grid;
				if ($GLOBALS["detail_nilai_grid"]->DetailAdd) {
					if ($this->CopyRecord)
						$GLOBALS["detail_nilai_grid"]->CurrentMode = "copy";
					else
						$GLOBALS["detail_nilai_grid"]->CurrentMode = "add";
					$GLOBALS["detail_nilai_grid"]->CurrentAction = "gridadd";

					// Save current master table to detail table
					$GLOBALS["detail_nilai_grid"]->setCurrentMasterTable($this->TableVar);
					$GLOBALS["detail_nilai_grid"]->setStartRecordNumber(1);
					$GLOBALS["detail_nilai_grid"]->id_nilai->FldIsDetailKey = TRUE;
					$GLOBALS["detail_nilai_grid"]->id_nilai->CurrentValue = $this->nilai_id->CurrentValue;
					$GLOBALS["detail_nilai_grid"]->id_nilai->setSessionValue($GLOBALS["detail_nilai_grid"]->id_nilai->CurrentValue);
				}
			}
		}
	}
...
// Create page object
if (!isset($nilai_add)) $nilai_add = new cnilai_add();
...
$nilai_add->Page_Render();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var nilai_add = new ew_Page("nilai_add");
nilai_add.PageID = "add"; // Page ID
var EW_PAGE_ID = nilai_add.PageID; // For backward compatibility

// Form object
var fnilaiadd = new ew_Form("fnilaiadd");

// Validate form
fnilaiadd.Validate = function() {
...
fnilaiadd.ValidateRequired = true;
<?php } else { ?>
fnilaiadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fnilaiadd.Lists["x_nis"] = {"LinkField":"x_id_siswa","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
fnilaiadd.Lists["x_nip"] = {"LinkField":"x_guru_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php $nilai_add->ShowPageHeader(); ?>
<?php
$nilai_add->ShowMessage();
?>
<form name="fnilaiadd" id="fnilaiadd" class="ewForm form-inline" action="<?php echo ew_CurrentPage() ?>" method="post">
<input type="hidden" name="t" value="nilai">
<input type="hidden" name="a_add" id="a_add" value="A">
<table class="ewGrid"><tr><td>
<table id="tbl_nilaiadd" class="table table-bordered table-striped">
...
<script type="text/javascript">
fnilaiadd.Lists["x_nis"].Options = <?php echo (is_array($nilai->nis->EditValue)) ? ew_ArrayToJson($nilai->nis->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<script type="text/javascript">
fnilaiadd.Lists["x_nip"].Options = <?php echo (is_array($nilai->nip->EditValue)) ? ew_ArrayToJson($nilai->nip->EditValue, 1) : "[]" ?>;
</script>
</span>
...
<input type="text" data-field="x_semester" name="x_semester" id="x_semester" size="30" maxlength="20" placeholder="<?php echo ew_HtmlEncode($nilai->semester->PlaceHolder) ?>" value="<?php echo $nilai->semester->EditValue ?>"<?php echo $nilai->semester->EditAttributes() ?>>
</span>
...
<input type="text" data-field="x_kelas" name="x_kelas" id="x_kelas" size="30" maxlength="20" placeholder="<?php echo ew_HtmlEncode($nilai->kelas->PlaceHolder) ?>" value="<?php echo $nilai->kelas->EditValue ?>"<?php echo $nilai->kelas->EditAttributes() ?>>
</span>
...
<?php
	if (in_array("detail_nilai", explode(",", $nilai->getCurrentDetailTable())) && $detail_nilai->DetailAdd) {
?>
<?php include_once "detail_nilaigrid.php" ?>
<?php } ?>
